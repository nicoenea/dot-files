...
// x \in R : x >= 0
#define VIGNETTE_BRIGHTNESS 9.0

// Tint all colors
// [0, 1]^3
#define TINT 0.85, 0.95, 0.90 
...
// [0, 1]
#define NOISE_UNIFORM_STRENGTH 0.01
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Detect and boost text before applying fade
vec3 backgroundColor = vec3(0.156, 0.164, 0.212); // Your background #282a36
float colorDifference = length(fragColor.rgb - backgroundColor);

// Boost text areas
if (colorDifference > 0.15) {
    fragColor.rgb *= 1.5; // 10% brighter for text areas
}
